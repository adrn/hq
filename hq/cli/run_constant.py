...
def get_robust_constant_model(data):
    with pm.Model() as const_model:
        ...
        like = pm.Normal("like", mu, err, observed=rv)
...
def get_robust_linear_model(data):
    with pm.Model() as linear_model:
        ...
        like = pm.Normal("like", a + b * t, err, observed=rv)
        pm.Deterministic("loglike", like.logp())
...
def worker(task):
    ...
    for source_id in source_ids:
        ...
        dt = (data.t.mjd - np.mean(data.t.mjd)) / 1000.0
        ...
        init_s = np.sqrt(
            np.sum((data.rv - np.mean(data.rv)) ** 2 - data.rv_err**2) / len(data)
        )
            ...
            try:
                with const_model:
                    const_map_p, const_info = pmx.optimize(
                        start={
                            "mu": np.mean(rv) + np.random.normal(0, 1e-2),
                            "lns": init_lns,
                        },
                        method="L-BFGS-B",
                        progress=False,
                ...
                logger.error(
                    f"FAILED: robust constant for source {source_id}\n"
                    f"{const_info}\n{e}"
                )
                const_map_p = None
            ...
            try:
                with linear_model:
                    linear_map_p, lin_info = pmx.optimize(
                        start={"a": np.mean(rv), "b": init_b, "lns": init_lns},
                        method="L-BFGS-B",
                        progress=False,
                        verbose=False,
                        return_info=True,
                ...
                logger.error(
                    f"FAILED: robust linear for source {source_id}\n" f"{lin_info}\n{e}"
                )
                linear_map_p = None

        # Non-robust / standard constant log-likelihood
        eval_mu = np.sum(rv / rv_err**2) / np.sum(1.0 / rv_err**2)
...
def run_constant(run_path, pool, overwrite=False):
    ...
    if c.constant_results_file.exists() and not overwrite:
        logger.info(
            f"Results file {c.constant_results_file!s} already "
