...
def worker(task):
    ...
    prior, model = conf.get_prior(P_min=task["P_range"][0], P_max=task["P_range"][1])
    ...
    rerun_samples = joker.rejection_sample(
        task["data"], prior_samples=conf.rerun_n_prior_samples, return_logprobs=True
    )
    ...
    if len(rerun_samples) > task["n_joker_samples"]:
        ...
        logger.warn(
            f"{prefix} Re-running TheJoker produced fewer samples than "
...
def callback(result):
    ...
    logger.debug(result["log_prefix"] + "Finished and writing results to results file")
