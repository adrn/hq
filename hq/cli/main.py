...
class CLI:
    ...
    def status(self):
        ...
        parser = get_parser(description=("TODO"), multiproc_options=False)
        # HACK
        ...
        if name is None:
            raise RuntimeError(
                "Failed to get status for HQ run! Check the "
    ...
    def make_prior_cache(self):
        ...
        with args.Pool(**args.Pool_kwargs) as pool:
            make_prior_cache(
                args.run_path, pool=pool, overwrite=args.overwrite, seed=args.seed
            )
    ...
    def run_thejoker(self):
        ...
        if args.seed is None:
            ...
            logger.log(1, f"No random seed specified, so using seed: {args.seed}")
    ...
    def run_constant(self):
        ...
        with threadpool_limits(limits=1, user_api="blas"):
            with args.Pool(**args.Pool_kwargs) as pool:
                run_constant(args.run_path, pool=pool, overwrite=args.overwrite)
    ...
    def rerun_thejoker(self):
        ...
        with threadpool_limits(limits=1, user_api="blas"):
            with args.Pool(**args.Pool_kwargs) as pool:
                rerun_thejoker(run_path=args.run_path, pool=pool, seed=args.seed)

        sys.exit(0)

    def run_mcmc(self):
        """Run MCMC (using pymc's NUTS sampler) for the unimodal samplings"""
        parser = get_parser(
            description=(
                ...
                "uses pymc and pytensor to run MCMC: As such, it needs to "
                "compile a pytensor model. By default, "
            )
        )
        ...
        pytensor_root = pathlib.Path(os.environ.get("HQ_PYTENSOR_PATH", "~/.pytensor/"))
        pytensor_root = pytensor_root.expanduser().absolute()

        # This horrifying set of hacks creates unique pytensor cache paths for
        ...
        # different processes try to update the pytensor cache at the same time.
        # These all need to happen before pytensor gets imported...
        pytensor_path = str(pytensor_root / f"mcmc{args.index}")

        if os.path.exists(pytensor_path):
            shutil.rmtree(pytensor_path)
        os.makedirs(pytensor_path)
        os.environ["PYTENSOR_FLAGS"] = f"base_compiledir={pytensor_path}"
        logger.info(f"pytensor flags set to: {os.environ['PYTENSOR_FLAGS']}")

        from .run_mcmc import run_mcmc

        run_mcmc(
            args.run_path, index=args.index, overwrite=args.overwrite, seed=args.index
        )
    ...
    def expand_samples(self):
        ...
        with threadpool_limits(limits=1, user_api="blas"):
            with args.Pool(**args.Pool_kwargs) as pool:
                expand_samples(args.run_path, pool=pool, overwrite=args.overwrite)
