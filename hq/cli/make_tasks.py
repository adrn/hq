...
def make_tasks(run_path, pool, overwrite=False):
    ...
    if c.tasks_file.exists() and not overwrite:
        logger.info(
            f"File {c.tasks_file!s} already exists: Use " "-o/--overwrite if needed"
        )
        return

    # Load the full data file, but store only some columns for speed:
    keys = [c.source_id_colname, c.rv_colname, c.rv_error_colname, c.time_colname]
    ...
    logger.info(
        f"Done preparing tasks: {len(source_ids)} sources in the " "processing queue"
    )
